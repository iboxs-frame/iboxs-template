...
namespace iboxs\template\taglib;

use iboxs\template\TagLib;
...
class Cx extends Taglib
{
    ...
    public function tagVolist(array $tag, string $content): string
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \iboxs\Collection || ' . $name . ' instanceof \iboxs\Paginator): $' . $key . ' = 0;';
    ...
    public function tagForeach(array $tag, string $content): string
    {
        ...
        $parseStr .= 'if(is_array(' . $name . ') || ' . $name . ' instanceof \iboxs\Collection || ' . $name . ' instanceof \iboxs\Paginator): ';
    ...
    public function tagEmpty(array $tag, string $content): string
    {
        $name     = $tag['name'];
        $name     = $this->autoBuildVar($name);
        $parseStr = '<?php if(empty(' . $name . ') || ((' . $name . ' instanceof \iboxs\Collection || ' . $name . ' instanceof \iboxs\Paginator ) && ' . $name . '->isEmpty())): ?>' . $content . '<?php endif; ?>';
