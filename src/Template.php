...
class Template
{
    ...
    public function parseTagLib(string $tagLib, string &$content, bool $hide = false): void
    {
        if (str_contains($tagLib, '\\')) {
            ...
            $className = '\\iboxs\\template\\taglib\\' . ucwords($tagLib);
        }
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (str_contains($match[0], '.')) {
                        ...
                        if (isset($this->extend[$first])) {
                            ...
                            // 所有以iboxs.打头的以特殊变量对待 无需模板赋值就可以输出
                            $parseStr = $this->parseiboxsVar($vars);
                        } else {
    ...
    /**
     * 特殊模板变量解析
     * 格式 以 $IBoxs. 打头的变量属于特殊模板变量
     * @access public
     * @param  array $vars 变量数组
     * @return string
     */
    public function parseiboxsVar(array $vars): string
    {
        $type  = strtoupper(trim(array_shift($vars)));
        $param = implode('.', $vars);
